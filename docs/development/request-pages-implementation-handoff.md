...
- The picker paginates locally in groups of 10 with Previous/Next controls. Its stock-status select supports all eligible items or **Low stock only**, defined as `quantityAvailable <= reorderLevel`, matching the existing Inventory low-stock presentation. Search and stock-filter changes reset to page 1; checkbox selection persists across picker pages.
