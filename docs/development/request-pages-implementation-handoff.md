...
- The picker paginates locally in groups of 5 with Previous/Next controls. Its stock-status select supports all eligible items or **Low stock only**, defined as `quantityAvailable <= reorderLevel`, matching the existing Inventory low-stock presentation. The compact select has no top margin and retains the accessible name `Stock status`. The picker table displays each item's `quantityAvailable` as an available-stock column. Search and stock-filter changes reset to page 1; checkbox selection persists across picker pages.
